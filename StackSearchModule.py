...
class StackAnswer:
    ...
    def __init__(self, link, body, username, title, score, profileImage):
        self.body = body
        self.link = link
...
async def fetchAnswer(session, id):
    """
...
async def findQuestion(session, query):
    """
...
async def Search(query):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        question = await findQuestion(session, query);

        # Grab the the first (top-voted) answer
        answerid = question.get("items", [{}])[0].get('accepted_answer_id', -1)

        # Grab the answer's JSON
        answer = None
        async with aiohttp.ClientSession() as sessionthing:
            answer = await fetchAnswer(sessionthing, answerid);
